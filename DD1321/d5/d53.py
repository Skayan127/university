...
for x in range(0,len(data)):    # Adds the words to the dictionary.
    tree.store(data[x].strip(),1)
tree.write()

with open("output.txt", "r") as text_file2:   # Reads the words from the file.
    data2 = text_file2.readlines()

lista=[]
for x in range(0,len(data2)):    # Adds the words to the dictionary.
    lista.append(data2[x].strip())
