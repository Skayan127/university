...
def parse_url_file(file_content):
    ...
    qq = veckadagdatum()
    veckovariabel=""#m.group(4)
    dagvariabel=""#m.group(1)
    datumvariabel=""
    ...
    for j, line in enumerate(lines):
        ...
        if (m != None) :
            ...
            if m.group(1)!="":
                dagvariabel = m.group(1)
            datumvariabel = m.group(3) + "/" + m.group(2)
            next
        ...
        if (m != None) : 
            ...
            qq.tid = m.group(1)
            qq.dag = dagvariabel
            qq.vecka = veckovariabel
            qq.datum = datumvariabel
            next
...
## IN  
##     
## OUT: Det som returneras, todo, är en dictionary.
##
...
## search_data
##
## IN: "what" är en sträng som kommer från parse_command_line_args "value".
##     
## OUT 
##
